...
export default async function handler(req, res) {
  ...
  const table = req.query?.table || parseBody(req).table;
  ...
  try {
    ...
    if (existing?.[0]?.id) {
      const result = await supabaseRest(table, { method: 'PATCH', query: { id: `eq.${existing[0].id}` }, body: row });
      return res.status(200).json(result?.[0] || row);
    }
    const result = await supabaseRest(table, { method: 'POST', body: row });
    return res.status(200).json(result?.[0] || row);
  } catch (error) {
